...
test('adicionar insere o item', () => {
  const c = criarCarrinho(storageFalso());
  assert.deepStrictEqual(c.adicionar('NI18'), [{ sku: 'NI18', qtd: 1 }]);
});
...
test('adicionar duas vezes soma a quantidade', () => {
  ...
  c.adicionar('NI18');
  c.adicionar('NI18', 2);
  assert.deepStrictEqual(c.ler(), [{ sku: 'NI18', qtd: 3 }]);
...
test('adicionar satura no máximo permitido', () => {
  ...
  c.adicionar('NI18', 19);
  c.adicionar('NI18', 10);
  assert.deepStrictEqual(c.ler(), [{ sku: 'NI18', qtd: 20 }]);
});
...
test('definirQtd troca a quantidade', () => {
  ...
  c.adicionar('NI18');
  c.definirQtd('NI18', 5);
  assert.deepStrictEqual(c.ler(), [{ sku: 'NI18', qtd: 5 }]);
});
...
test('remover tira só o item pedido', () => {
  ...
  c.adicionar('NI9PRO');
  c.remover('NI18');
  assert.deepStrictEqual(c.ler(), [{ sku: 'NI9PRO', qtd: 1 }]);
});
...
test('carrinho persiste entre instâncias com o mesmo storage', () => {
  ...
  criarCarrinho(s).adicionar('NI23', 2);
  assert.deepStrictEqual(criarCarrinho(s).ler(), [{ sku: 'NI23', qtd: 2 }]);
});
...
test('storage com formato inesperado é descartado', () => {
  const c = criarCarrinho(storageFalso({ 'nuvemair.carrinho': '{"sku":"NI18"}' }));
  assert.deepStrictEqual(c.ler(), []);
});

test('itens malformados são filtrados na leitura', () => {
  const s = storageFalso({ 'nuvemair.carrinho': '[{"sku":"NI18","qtd":2},{"qtd":3},{"sku":"NI9PRO","qtd":0}]' });
  assert.deepStrictEqual(criarCarrinho(s).ler(), [{ sku: 'NI18', qtd: 2 }]);
});
...
test('storage indisponível não derruba a página', () => {
  ...
  assert.deepStrictEqual(c.ler(), []);
  assert.doesNotThrow(() => c.adicionar('NI18'));
});
